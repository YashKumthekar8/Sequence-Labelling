...
def load_pretrain_emb(embedding_path):
    embedd_dim = -1
    embedd_dict = dict()
    hindi_paths = ['hindi_pos/test_hindi.pos', 'hindi_pos/train_hindi.pos', 'hindi_pos/dev_hindi.pos']
    marathi_paths = ['marathi_pos/test_marathi.pos', 'marathi_pos/train_marathi.pos', 'marathi_pos/dev_marathi.pos']
    if embedding_path == 'glove.6B.100d.txt':
    ...
    elif embedding_path == 'test_hindi.pos':
        ft = fasttext.load_model("indicnlp.ft.hi.300.bin")
        for path in hindi_paths:
            with open(path, 'r', encoding='utf8') as file:
                for line in file:
                    if len(line) < 5:
                        continue
                    word = line.strip()[0]
                    tokens = ft.get_word_vector(word)
                    if embedd_dim < 0:
                        embedd_dim = len(tokens) - 1
                    else:
                        assert (embedd_dim + 1 == len(tokens))
