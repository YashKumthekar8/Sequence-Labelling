...
def evaluate(data, model, name, nbest=None):
    ...
    for batch_id in range(total_batch):
        ...
        if not instance:
            continue
        batch_word, batch_wordlen, batch_wordrecover, batch_char, batch_charlen, batch_charrecover, batch_label, mask, input_label_seq_tensor = batchify_with_label(instance, data.HP_gpu, data.label_alphabet_size)
        ...
        if nbest:
            scores, nbest_tag_seq = model.decode_nbest(batch_word, batch_wordlen, batch_char, batch_charlen, batch_charrecover, mask, nbest,input_label_seq_tensor)
            ...
            tag_seq = model(batch_word, batch_wordlen, batch_char, batch_charlen, batch_charrecover, mask,input_label_seq_tensor)


        pred_label, gold_label = recover_label(tag_seq, batch_label, mask, data.label_alphabet, batch_wordrecover)
        # print(pred_label[0], gold_label[0])
...
def batchify_with_label(input_batch_list, gpu,label_size = 20, volatile_flag=False):
    ...
    return word_seq_tensor, word_seq_lengths, word_seq_recover, char_seq_tensor, char_seq_lengths, char_seq_recover, label_seq_tensor, mask ,input_label_seq_tensor
...
def train(data):
    ...
    for idx in range(data.HP_iteration):
        ...
        for batch_id in range(total_batch):
            ...
            instance_count += 1
            loss, tag_seq = model.neg_log_likelihood_loss(batch_word, batch_wordlen, batch_char, batch_charlen, batch_charrecover, batch_label, mask,input_label_seq_tensor)
        ...
        if total_loss > 1e8 or str(total_loss) == "nan":
            ...
            #exit(1)
        # continue
        speed, acc, p, r, f, prediction_f,_ = evaluate(data, model, "dev")
        ...
        if data.seg:
            ...
            # print("Dev: time: %.2fs, speed: %.2fst/s; acc: %.4f, p: %.4f, r: %.4f, f: %.4f"%(dev_cost, speed, acc, p, r, f))
            # print(f'Predicted Sequence : {prediction_f}')

        # ## decode test
        speed, acc_test, p, r, f_test, prediction_t,_ = evaluate(data, model, "test")
        ...
        if data.seg:
            ...
            print("Test: time: %.2fs, speed: %.2fst/s; acc: %.4f, p: %.4f, r: %.4f, f: %.4f"%(test_cost, speed, acc_test, p, r, f_test))
            # print(f'Predicted Sequence : {prediction_t}')
    ...
    data_ = open('colnll_2003/ner_dev.ner', 'r').readlines()
    actual_labels = []
    for lines in data_:
        line = lines.strip().split()
        actual_labels.append(line[-1])

    cm = confusion_matrix(actual_labels, prediction_f)
    import pandas as pd
    df = pd.DataFrame(cm)
    df.to_csv('Confusion_Matrix_Dev.csv')


    data_ = open('colnll_2003/ner_test.ner', 'r').readlines()
    actual_labels = []
    for lines in data_:
        line = lines.strip().split()
        actual_labels.append(line[-1])

    cm = confusion_matrix(actual_labels, prediction_t)
    df = pd.DataFrame(cm)
    df.to_csv('Confusion_Matrix_Test.csv')
